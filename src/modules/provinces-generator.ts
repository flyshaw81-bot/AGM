    ...
    try {
      ...
      states.forEach((s) => {
        ...
        const stateBurgs = burgs
          ...
          .sort(
            (a, b) =>
              b.population! * gauss(1, 0.2, 0.5, 1.5, 3) - a.population!,
      ...
      for (const i of cells.i) {
        ...
        const neibs = cells.c[i]
          .filter(
            (c) =>
              cells.state[c] === cells.state[i] && !isProvinceCellLocked(c),
          )
          .map((c) => provinceIds[c]);
