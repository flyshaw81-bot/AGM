import {
  getNativeRelationshipButtonFieldValue,
  getNativeRelationshipCurrentFieldValue,
  getNativeRelationshipFixField,
  getNativeRelationshipFixTarget,
  type NativeRelationshipQueueItem,
  normalizeRelationshipQueueValue,
} from "./nativeRelationshipQueue";

type CreateNativeRelationshipQueueItemOptions = {
  existingItems: readonly NativeRelationshipQueueItem[];
  fallbackLabel: string;
  fixButton: HTMLElement;
};

const RELATIONSHIP_PREVIEW_SEPARATOR = /\s*(?:->|=>|→|鈫\?|閳玕?|鈫抾->)\s*/u;

export function parseNativeRelationshipQueuePreview(preview: string) {
  const [source = "", target = ""] = preview
    .split(RELATIONSHIP_PREVIEW_SEPARATOR, 2)
    .map((value) => normalizeRelationshipQueueValue(value || ""));
  return { source, target };
}

export function createNativeRelationshipQueueItem({
  existingItems,
  fallbackLabel,
  fixButton,
}: CreateNativeRelationshipQueueItemOptions): NativeRelationshipQueueItem | null {
  const { fixKind, entity, id } = getNativeRelationshipFixTarget(fixButton);
  if (!entity || !id) return null;

  const field = getNativeRelationshipFixField(fixKind);
  const key = `${fixKind}:${entity}:${id}`;
  if (existingItems.some((item) => item.key === key)) return null;

  const preview =
    fixButton
      .closest<HTMLElement>(".studio-direct-workbench-directory__issue")
      ?.querySelector<HTMLElement>(
        "[data-direct-relationship-preview='true'] code",
      )
      ?.textContent?.trim() ||
    fixButton.textContent?.trim() ||
    fallbackLabel;
  const { source: previewSource, target: previewTarget } =
    parseNativeRelationshipQueuePreview(preview);
  const label =
    fixButton
      .closest<HTMLElement>(".studio-direct-workbench-directory__issue")
      ?.querySelector<HTMLElement>(
        ".studio-direct-workbench-directory__issue-title",
      )
      ?.textContent?.trim() || fallbackLabel;

  return {
    key,
    fieldKey: `${entity}:${id}:${field}`,
    label,
    preview,
    target: fixButton.dataset.workbenchTarget || "",
    sourceValue:
      previewSource ||
      getNativeRelationshipCurrentFieldValue(entity, id, field),
    targetValue:
      previewTarget ||
      getNativeRelationshipButtonFieldValue(fixButton, entity, field),
    button: fixButton.cloneNode(true) as HTMLElement,
  };
}
